...
grammar_file = open('2stress.txt', 'r')
...
def ranked_tuples_list(const_dict, rnd=False):
    ranked_list_raw=[]
    for const in const_dict:
        ranked_list_raw.append((const, const_dict[const]))
    ranked_list_raw = sorted(ranked_list_raw, key=lambda x: x[1], reverse=True)
    ...
    for c in ranked_list:
        if rnd==False:
            ranked_tuples.append((c, const_dict[c]))
        else:
            ranked_tuples.append((c, round(const_dict[c], 2)))
...
def learn_iter(input_parse, rip_parse, const):
    if input_parse == rip_parse:
        return
    else:
        pass 
...
init_constraint_dict={}

for c in consts:
    init_constraint_dict[c] = 100.0

init_constraint_dict = initialize_grammar(init_constraint_dict)

#print(learn(overt_tableaux["[L1 L]"], input_tableaux["|L L|"], constraint_dict))

'''
script_path = os.path.dirname(os.path.realpath(sys.argv[0])) #<-- absolute dir the script is in
results_path = script_path + '\\results'
result_file_name = "\\"+lang+"_"+str(iter_limit)+"iter_"+timestamp+".txt"
...
constraint_dict = init_constraint_dict


for t in target_list_shuffled:
    gen = generate(get_input(t), ranking(constraint_dict))
    parse = rip(t, ranking(constraint_dict))

    if gen != parse:
        print(t+":")
        print("Gen: "+gen[0]+", Parse: "+parse[0])

        sleep(0.5)

        print(overt_tableaux[t][parse[0]])
        print(input_tableaux[get_input(t)][gen[0]])

        sleep(0.5)
        
        constraint_dict = learn(parse[1], gen[1], constraint_dict)
        gen = generate(get_input(t), ranking(constraint_dict))
        parse = rip(t, ranking(constraint_dict))

        print("Current ranking: ")

        print(ranked_tuples_list(constraint_dict, rnd=True))
